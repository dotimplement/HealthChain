...
def write_model(file, name: str, model_info: Dict) -> Dict[str, List[str]]:
    ...
    if "properties" in model_info:
        ...
        description = (
            model_info.get("description").replace('"', "").split("\n")[0].split("\r")[0]
        )
...
def write_field(
    file, prop_name: str, prop_info: Dict, prop_description: str, name: str
):
    if "$ref" in prop_info:
        ...
        if ref_model == "base64Binary":
            ref_model = "string"
        file.write(
            f'    {prop_name}_field: {ref_model}Model = Field(default=None, alias="{prop_name}", description="{prop_description}")\n'
        )
    elif "type" in prop_info and "items" in prop_info and "$ref" in prop_info["items"]:
        item_model = prop_info["items"]["$ref"].split("/")[-1]
        file.write(
            f'    {prop_name}_field: List[{item_model}Model] = Field(default_factory=list, alias="{prop_name}", description="{prop_description}")\n'
        )
    elif "enum" in prop_info:
        file.write(
            f'    {name}_{prop_name}_field: {prop_name}Model = Field(..., alias="{prop_name}", description="{prop_description}")\n'
        )
