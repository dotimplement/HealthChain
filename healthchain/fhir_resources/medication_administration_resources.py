...
class MedicationAdministration_PerformerModel(BaseModel):
    ...
    actor_field: CodeableReferenceModel = Field(
        default=None,
        alias="actor",
...
class MedicationAdministration_DosageModel(BaseModel):
    id_field: stringModel = Field(
        default=None,
        alias="id",
        description="Unique id for the element within a resource (for internal references). This may be any string value that does not contain spaces.",
    ...
    method_field: CodeableConceptModel = Field(
        default=None,
        alias="method",
    ...
    dose_field: QuantityModel = Field(
        default=None,
        alias="dose",
    ...
    rateQuantity_field: QuantityModel = Field(
        default=None,
        alias="rateQuantity",
        description="Identifies the speed with which the medication was or will be introduced into the patient.  Typically, the rate for an infusion e.g. 100 ml per 1 hour or 100 ml/hr.  May also be expressed as a rate per unit of time, e.g. 500 ml per 2 hours.  Other examples:  200 mcg/min or 200 mcg/1 minute; 1 liter/8 hours.",
    )
...
class MedicationAdministrationModel(BaseModel):
    resourceType_field: str = "MedicationAdministration"
    ...
    statusReason_field: List[CodeableConceptModel] = Field(
        default_factory=list,
        alias="statusReason",
    ...
    medication_field: CodeableReferenceModel = Field(
        default=None,
        alias="medication",
    ...
    occurencePeriod_field: PeriodModel = Field(
        default=None,
    ...
    subPotentReason_field: List[CodeableConceptModel] = Field(
        default_factory=list,
        alias="subPotentReason",
        description="The reason or reasons why the full dose was not administered.",
    )
    performer_field: List[MedicationAdministration_PerformerModel] = Field(
        default_factory=list,
        alias="performer",
    ...
    request_field: ReferenceModel = Field(
        default=None,
        alias="request",
