...
from healthchain.data_generator.encounter_generator import ClassGenerator, EncounterTypeGenerator, EncounterGenerator


def test_ClassGenerator():
    patient_class = ClassGenerator.generate()
    assert patient_class.coding_field[0].system_field == "http://terminology.hl7.org/CodeSystem/v3-ActCode"
...
def test_EncounterTypeGenerator():
    ...
    assert encounter_type.coding_field[0].system_field == "http://terminology.hl7.org/CodeSystem/v3-ActCode"
...
def test_EncounterModel():
    ...
    assert encounter.status_field in ('planned', 'in-progress', 'on-hold', 'discharged', 'cancelled')
